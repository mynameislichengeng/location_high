...
//                .setRequiredNetworkType(NetworkType.CONNECTED)
        ...
        PeriodicWorkRequest periodicWorkRequest = new PeriodicWorkRequest.Builder(
                MyLocationPeriodicWorker.class,
                30, TimeUnit.SECONDS
        )
                .setConstraints(constraints)  // 设置网络约束
                .build();

        // build使用 WorkManager 进行调度
        WorkManager.getInstance(this).enqueue(periodicWorkRequest);

        // 创建一个一次性任务，用于确保在设备重启后能够重新调度周期性任务
        OneTimeWorkRequest oneTimeWorkRequest = new OneTimeWorkRequest.Builder(MyLocationPeriodicWorker.class)
                .setInitialDelay(1, TimeUnit.MINUTES)  // 设定初始延迟时间
                .setBackoffCriteria(BackoffPolicy.EXPONENTIAL, 1, TimeUnit.MINUTES)  // 设置指数退避策略
                .build();
        // 使用 WorkManager 进行调度
